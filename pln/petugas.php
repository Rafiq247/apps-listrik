...
if ($petugas) {
	$substring = substr($petugas['id_petugas'], 9, 3);
	if (is_numeric($substring)) {
		$kode = (int)$substring + 1;
	} else {
		$kode = 1; // Jika substring tidak numerik, mulai dari 1
	}
} else {
	$kode = 1; // Jika tidak ada data petugas, mulai dari 1
}
