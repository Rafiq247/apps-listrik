<?php
// Periksa status sesi dan mulai sesi jika belum dimulai
if (session_status() == PHP_SESSION_NONE) {
	session_start();
}

// Pastikan jalur ke file koneksi.php benar
include '../config/koneksi.php'; // untuk koneksi ke database

// Redirect jika parameter menu tidak ada
if (!isset($_GET['menu'])) {
	header('location:hal_utama.php?menu=laporan');
	exit();
}

// Variabel bulan dan tahun dari form POST
$bulanini = isset($_POST['bulan']) ? $_POST['bulan'] : '';
$tahunini = isset($_POST['tahun']) ? $_POST['tahun'] : '';

// Cek form post bulan dan tahun
if (!empty($bulanini) && !empty($tahunini)) {
	$cari = "WHERE MONTH(tgl_bayar) = '$bulanini' AND YEAR(tgl_bayar) = '$tahunini' AND id_petugas = '$_SESSION[id_petugas]'";
} else {
	$cari = ''; // Set default cari ke string kosong jika tidak ada bulan dan tahun
}
?>
				...
				?>
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							<center><label>LAPORAN TAGIHAN <?php echo strtoupper(@$status) . " BULAN ";
															$aksi->bulan(@$bulanini);
										...
										<tbody>
											...
											if (empty($data)) {
												...
												foreach ($data as $r) {
													...
													if ($cek >= 3) {
														...
														$stmt_sum = $koneksi->prepare("SELECT id_pelanggan, COUNT(bulan) as bln_tunggak, SUM(jumlah_bayar) as jml_bayar, SUM(jumlah_meter) as jml_meter, tarif_perkwh FROM tagihan WHERE id_pelanggan = ? AND status = 'Belum Bayar'");
														$stmt_sum->bind_param("s", $r['id_pelanggan']);
														$stmt_sum->execute();
														$result_sum = $stmt_sum->get_result();
														$sum = $result_sum->fetch_assoc();

														$stmt_bulan = $koneksi->prepare("SELECT * FROM tagihan WHERE id_pelanggan = ? AND status = 'Belum Bayar'");
														$stmt_bulan->bind_param("s", $r['id_pelanggan']);
														$stmt_bulan->execute();
											...
											?>
														<tr>
															...
															<td align="center">
																<?php
																while ($bln = $result_bulan->fetch_assoc()) {
							...
							<div class="col-md-12">
								<form method="post">
									<div class="input-group">
										...
										<input type="text" name="id_pelanggan" class="form-control" placeholder="Masukan ID Pelanggan" value="<?php echo htmlspecialchars(@$id_pelanggan); ?>" list="id_pel" onkeypress='return event.charCode >=48 && event.charCode <=57' <?php if (@$_GET['id']) {
																																																																				echo "readonly";
																																																																			} ?>>
										<datalist id="id_pel">
											<?php
											...
											while ($b = $result->fetch_assoc()) { ?>
												<option value="<?php echo htmlspecialchars($b['id_pelanggan']); ?>"><?php echo htmlspecialchars($b['nama']); ?></option>
											<?php } ?>
										</datalist>
										<div class="input-group-addon" id="pri">Tahun</div>
										<select name="tahun" class="form-control">
									...
									<table class="table table-bordered table-striped table-hover">
										...
										<tbody>
											...
											?>
													<tr>
														<td align="center"><?php echo $no; ?>.</td>
														<td align="center"><?php echo htmlspecialchars($r['id_pelanggan']); ?></td>
														<td align="left"><?php echo htmlspecialchars($r['nama_pelanggan']); ?></td>
														<td align="center"><?php $aksi->bulan($r['bulan']);
																			echo " " . htmlspecialchars($r['tahun']); ?></td>
														<td align="center"><?php echo htmlspecialchars($penggunaan['meter_awal']); ?></td>
														<td align="center"><?php echo htmlspecialchars($penggunaan['meter_akhir']); ?></td>
														<td align="center"><?php echo htmlspecialchars($r['jumlah_meter']); ?></td>
														<td align="right"><?php $aksi->rupiah($r['tarif_perkwh']); ?></td>
														<td align="right"><?php $aksi->rupiah($r['jumlah_bayar']); ?></td>
													</tr>
											<?php
											...
											// Menggunakan MySQLi untuk query sum
											if (isset($id_pelanggan) && isset($tahun)) {
												$query_sum = "SELECT SUM(jumlah_meter) as meter, SUM(jumlah_bayar) as bayar FROM tagihan WHERE id_pelanggan = '$id_pelanggan' AND tahun = '$tahun'";
												$result_sum = $koneksi->query($query_sum);
												$sum = $result_sum->fetch_assoc();
											} else {
												$sum = ['meter' => 0, 'bayar' => 0];
											}
