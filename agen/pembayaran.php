...
// Koneksi ke database
include '../config/koneksi.php'; // Pastikan Anda sudah mengimpor file koneksi Anda

// Kode otomatis
$hari_ini = date("Ymd");

// Siapkan pernyataan SQL
$stmt = $koneksi->prepare("SELECT id_pembayaran FROM pembayaran WHERE id_pembayaran LIKE CONCAT('%', ?, '%') ORDER BY id_pembayaran DESC");
$stmt->bind_param("s", $hari_ini);
$stmt->execute();
$result = $stmt->get_result();
$cek = $result->fetch_assoc();

// Menentukan ID pembayaran
if (empty($cek)) {
		...
		<div class="row">
							...
							<div class="panel-body">
								<form method="post">
									...
									<div class="input-group">
										<input type="text" name="id_pelanggan" class="form-control" value="<?php echo isset($_GET['id_pelanggan']) ? $_GET['id_pelanggan'] : @$id_pelanggan; ?>" placeholder="Masukan ID Pelanggan ...." onkeypress='return event.charCode >=48 && event.charCode <=57' list="list">
										<datalist id="list">
											<?php
											// Menggunakan MySQLi untuk mengambil data pelanggan
											$a = $koneksi->query("SELECT * FROM pelanggan");
			...
			?>
										...
										<div class="table-responsive">
											<table class="table table-bordered table-striped table-hover">
												...
												<tbody>
													<?php
													$no = 0;
													$id_pelanggan = $_POST['id_pelanggan']; // Pastikan $id_pelanggan diambil dari input yang sesuai

													// Mengambil data tagihan menggunakan MySQLi
													$data = $aksi->tampil("tagihan", "WHERE id_pelanggan = '$id_pelanggan' AND status = 'Belum Bayar'", "ORDER BY bulan ASC");
													if ($data == "") {
														$aksi->no_record(8);
													} else {
														foreach ($data as $r) {
															// Mengambil data penggunaan menggunakan MySQLi
													...
													?>
															<tr>
																...
																<td align="center">
																	<a href="?menu=pembayaran&id_pelanggan=<?php echo $r['id_pelanggan']; ?>&bulan=<?php echo $r['bulan']; ?>&tahun=<?php echo $r['tahun']; ?>" class="btn btn-success btn-lg btn-block" target="_blank">BAYAR</a>
																</td>
															</tr>
													<?php
														}
													...
													// Mengambil total tagihan yang belum dibayar menggunakan MySQLi
													$stmt_total = $koneksi->prepare("SELECT SUM(jumlah_bayar) AS sum_bayar FROM tagihan WHERE id_pelanggan = ? AND status = 'Belum Bayar'");
													$stmt_total->bind_param("s", $id_pelanggan);
													$stmt_total->execute();
													$result_total = $stmt_total->get_result();
													$sum_total = $result_total->fetch_assoc();
													?>
												</tbody>
				...
				<div class="col-md-12">
					...
					<div class="col-md-6">
						<div class="panel panel-default">
							<div class="panel-heading">PEMBAYARAN - <?php echo @$_GET['id_pelanggan'] . " Bulan ";
